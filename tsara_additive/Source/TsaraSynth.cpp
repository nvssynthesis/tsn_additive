...
:	navPhasor(*this)
{
	essentia::Pool const p = file2pool("/Users/nicholassolem/development/audio for analysis/sine, saw, noise.yaml", "yaml");
	loadPool(p);
...
void TsaraSynth::TsaraVoice::renderNextBlock (juce::AudioBuffer<float>& outputBuffer,
	...
	if (shouldSelectNewFrame){
		nvs::DirectedGraph_t::vertex_descriptor vd = nvs::traverseToRandomVertex(*dg, *initialTargetVit);
	...
	requestNextTargetFrame(initialTargetFrame);
